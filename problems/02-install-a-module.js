...
Let's start out by installing the "@isaacs/once" module.

Run `$ADVENTURE_COMMAND verify` once you're done.
*/}.toString().split('\n').slice(1,-1).join('\n')
}

//exports.solution = 'npm install @isaacs/once'
...
exports.verify = function (args, cb) {
  ...
  try {
    var once = require(cwd + '/node_modules/@isaacs/once')
  } catch (er) {
    console.log('Uh oh!  Looks like it didn\'t install right.\n'+
                'The error I got was: \n' +
                (er.stack || er.message) + '\n' +
                'Make sure you use the `npm install @isaacs/once` command\n' +
                'to install the `@isaacs/once` module.')
    return cb(false)
  }
