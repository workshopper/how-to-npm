...
function handler (args) {
  ...
  return function (req, res) {
    ...
    switch (lesson) {
      ...
      case 'dist-tag':
        if (req.url.match(/^\/-\/package\/[^\/]+\/dist-tags/))
          return handleDistTags(req, res)
        break
    }
...
function handleDistTags (req, res) {
  console.log('dist-tag', req.method, req.url)
  ...
  if (req.method === 'POST' || req.method === 'PUT') {
    ...
    return req.pipe(concat(function (body) {
      ...
      if (tag) {
        ...
        if (req.method === 'PUT') {
          data['dist-tags'] = body
        } else for (var dt in body) {
          data['dist-tags'][dt] = body[dt]
        }
      }

      for (var dt in data['dist-tags']) {
        if (semver.valid(dt)) {
          return _403(req, res, {
  ...
  if (req.method === 'DELETE') {
    ...
    console.log('delete a tag', req.url)
...
function receivePublishPut (req, res) {
  ...
  function next(data) {
    ...
    att = att && att.data
    var dir = path.resolve(assetdir, querystring.escape(data._id))
    var jsonFile = path.resolve(dir, 'body.json')
    var tgzDir = path.resolve(dir, '-')
    var tgzFile = path.resolve(tgzDir, querystring.escape(tgzBase))
...
function static (req, res) {
  ...
  try {
    ...
    try {
      var body = fs.readFileSync(path.resolve(file, 'body.json'))
      var isJson = true
    } catch (er) {
      return _500(req, res, er)
    }
